      subroutine ZZmassiveboxtri(j1,j2,j3,j4,j5,j6,za,zb,sprod,mass,
     & totrat,box,tri)
      use mod_types; use mod_consts_dp
c--- coefficients of boxes and triangles using the integral basis
c--- that includes 6-dimensional boxes
c--- (-,+) and (+,-) helicities of gluons only
      implicit none
c     include 'docheck.f'
      include 'ggZZintegrals.f'
      include 'ZZdlabels.f'
      include 'ZZclabels.f'
c     include 'first.f'
      integer j1,j2,j3,j4,j5,j6,h1,h2,h3,h4,j,itot,irat
      real(dp)  :: mass,masssq
      complex(dp)  :: Xpp(2,2),Xmp(2,2),Xpm(2,2),Xmm(2,2),
     & Zmp(2,2,2),Zpm(2,2,2),totrat(2,2,2,2),
     & box(2,2,2,2,-2:0),tri(2,2,2,2,-2:0),d(2,2,2,2,5),c(2,2,2,2,6),
     & Xrat(2,2,2,2),drat(2,2,2,2,3),crat(2,2,2,2,6)
      complex(dp)  :: za(6,6),zb(6,6)
      real(dp)  :: sprod(6,6)
      parameter(itot=1,irat=2)
      
      masssq=mass**2
      
c      if (first) then
c      write(6,*) 'masssq',masssq
c      write(6,*) 'musq',musq
c        first=.false. 
c        call qlinit
c      pause
c      endif


c--- Box 1: note that coefficient of this box is unchanged
      call ZZbox1LL(j1,j2,j3,j4,j5,j6,za,zb,sprod,mass,
     & Xpp,Xmp,Xpm,Xmm,Xrat)
      d(1,2,:,:,d1_34_2)=Xmp(:,:)
      d(2,1,:,:,d1_34_2)=Xpm(:,:)


c--- d=6 Box 2
      call ZZD062x1x34LLmp(j1,j2,j3,j4,j5,j6,za,zb,sprod,mass,
     & Xmp,Xpm)
      d(1,2,:,:,d6_2_1_34)=Xmp(:,:)
      d(2,1,:,:,d6_2_1_34)=Xpm(:,:)
      
c--- d=6 Box 3
      call ZZD062x1x34LLmp(j2,j1,j3,j4,j5,j6,za,zb,sprod,mass,
     & Xmp,Xpm)
      d(1,2,:,:,d6_1_2_34)=Xpm(:,:)
      d(2,1,:,:,d6_1_2_34)=Xmp(:,:)


c---- From here on, returned arrays include complete result and
c---- also the contribution of that integral to the rational part
c---- (note change from Xmp,Xpm to Zmp,Zpm)

c---- Box 2
      call ZZD02x1x34LLmp(j1,j2,j3,j4,j5,j6,za,zb,sprod,mass,Zmp,Zpm)
      d(1,2,:,:,d2_1_34)=Zmp(:,:,itot)
      d(2,1,:,:,d2_1_34)=Zpm(:,:,itot)
      drat(1,2,:,:,d2_1_34)=Zmp(:,:,irat)
      drat(2,1,:,:,d2_1_34)=Zpm(:,:,irat)


c---- Box 3
      call ZZD02x1x34LLmp(j2,j1,j3,j4,j5,j6,za,zb,sprod,mass,Zmp,Zpm)
      d(1,2,:,:,d1_2_34)=Zpm(:,:,itot)
      d(2,1,:,:,d1_2_34)=Zmp(:,:,itot)
      drat(1,2,:,:,d1_2_34)=Zpm(:,:,irat)
      drat(2,1,:,:,d1_2_34)=Zmp(:,:,irat)
   
       
c---- Triangle 1
      call ZZC01x2LLmp(j1,j2,j3,j4,j5,j6,za,zb,sprod,mass,Zmp,Zpm)
      c(1,2,:,:,c1_2)=Zmp(:,:,itot)
      c(2,1,:,:,c1_2)=Zpm(:,:,itot)
      crat(1,2,:,:,c1_2)=Zmp(:,:,irat)
      crat(2,1,:,:,c1_2)=Zpm(:,:,irat)
      
c---- Triangle 3
      call ZZC01x34LLmp(j1,j2,j3,j4,j5,j6,za,zb,sprod,mass,Zmp,Zpm)
      c(1,2,:,:,c1_34)=Zmp(:,:,itot)
      c(2,1,:,:,c1_34)=Zpm(:,:,itot)
      crat(1,2,:,:,c1_34)=Zmp(:,:,irat)
      crat(2,1,:,:,c1_34)=Zpm(:,:,irat)

c---- Triangle 4
      call ZZC01x34LLmp(j2,j1,j3,j4,j5,j6,za,zb,sprod,mass,Zmp,Zpm)
      c(1,2,:,:,c2_34)=Zpm(:,:,itot)
      c(2,1,:,:,c2_34)=Zmp(:,:,itot)
      crat(1,2,:,:,c2_34)=Zpm(:,:,irat)
      crat(2,1,:,:,c2_34)=Zmp(:,:,irat)

c---- Triangle 5
      call ZZC01x34LLmp(j1,j2,j5,j6,j3,j4,za,zb,sprod,mass,Zmp,Zpm)
      do h3=1,2
      do h4=1,2
      c(1,2,h3,h4,c1_56)=Zmp(h4,h3,itot)
      c(2,1,h3,h4,c1_56)=Zpm(h4,h3,itot)
      crat(1,2,h3,h4,c1_56)=Zmp(h4,h3,irat)
      crat(2,1,h3,h4,c1_56)=Zpm(h4,h3,irat)
      enddo
      enddo

c---- Triangle 6
      call ZZC01x34LLmp(j2,j1,j5,j6,j3,j4,za,zb,sprod,mass,Zmp,Zpm)
      do h3=1,2
      do h4=1,2
      c(1,2,h3,h4,c2_56)=Zpm(h4,h3,itot)
      c(2,1,h3,h4,c2_56)=Zmp(h4,h3,itot)
      crat(1,2,h3,h4,c2_56)=Zpm(h4,h3,irat)
      crat(2,1,h3,h4,c2_56)=Zmp(h4,h3,irat)
      enddo
      enddo

c---- Triangle 2      

c------ Fill the coefficients for the massless case
      call ZZC012x34LLmp(j1,j2,j3,j4,j5,j6,za,zb,sprod,mass,Zmp,Zpm)
      c(1,2,:,:,c12_34)=Zmp(:,:,itot)
      c(2,1,:,:,c12_34)=Zpm(:,:,itot)
      crat(1,2,:,:,c12_34)=Zmp(:,:,irat)
      crat(2,1,:,:,c12_34)=Zpm(:,:,irat)

c------ Compute mass^2 dependence from rational contributions and add
      do h1=1,2
      h2=3-h1
      do h3=1,2
      do h4=1,2
c        write(6,*) h1,h2,h3,h4,crat(h1,h2,h3,h4,c12_34)
        crat(h1,h2,h3,h4,c12_34)=totrat(h1,h2,h3,h4)
     &    -3d0*drat(h1,h2,h3,h4,d2_1_34)-crat(h1,h2,h3,h4,c1_2)
     &    -crat(h1,h2,h3,h4,c1_34)-crat(h1,h2,h3,h4,c2_34)
     &    -crat(h1,h2,h3,h4,c1_56)-crat(h1,h2,h3,h4,c2_56)
c        write(6,*) h1,h2,h3,h4,crat(h1,h2,h3,h4,c12_34)
c        write(6,*)
        c(h1,h2,h3,h4,c12_34)=c(h1,h2,h3,h4,c12_34)
     &   +2d0*masssq*crat(h1,h2,h3,h4,c12_34)
      enddo
      enddo
      enddo

c      if (docheck) then
c        do h3=1,2
c        do h4=1,2
c        call ggZZcapture('12x34',h3,h4,1,2,3,4,5,6,
c     &   c(1,2,h3,h4,c12_34)-2d0*masssq*crat(1,2,h3,h4,c12_34),
c     &   +2d0*crat(1,2,h3,h4,c12_34),czero)
c        enddo
c        enddo
c      endif

c--- only compute finite part here
      do h1=1,2
      h2=3-h1
      box(h1,h2,:,:,:)=czero
      tri(h1,h2,:,:,:)=czero
      do h3=1,2
      do h4=1,2
c---  boxes
      do j=1,5
      box(h1,h2,h3,h4,0)=box(h1,h2,h3,h4,0)+d(h1,h2,h3,h4,j)*intD0(j)
      enddo
c---  triangles
      do j=1,6
      tri(h1,h2,h3,h4,0)=tri(h1,h2,h3,h4,0)+c(h1,h2,h3,h4,j)*intC0(j)
      enddo
      enddo
      enddo
      enddo

      return
      end
      
