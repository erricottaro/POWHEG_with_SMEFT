      subroutine ZZtri1_34LL(j1,j2,j3,j4,j5,j6,za,zb,sprod,mass,
...
C----Functions for the pp amplitude
     ...
     & *(sprod(k1,k3)+sprod(k1,k4))/
     & (2d0*za(k1,k2)**4*za(k3,k4)*za(k5,k6)))
...
C----Functions for the mp amplitude
     ...
     & +0.5d0*(za(k1,k5)**2*za(k3,k4)*zb(k4,k1)**2)
     & /(za(k5,k6)*zab2(k2,k3,k4,k1)**2)

     & -(t(k1,k3,k4)*za(k1,k2)*za(k1,k3)*zb(k4,k1)
     & *zab2(k5,k3,k4,k1)**2)
     & /(za(k5,k6)*(sprod(k1,k3)+sprod(k1,k4))*zab2(k2,k3,k4,k1)**3)

     &+0.5d0*(za(k3,k5)*(sprod(k1,k3)+sprod(k1,k4))*zab2(k5,k3,k4,k1)
     &     *zb(k4,k2))/(za(k5,k6)*zb(k2,k1)*zab2(k2,k3,k4,k1)**2)
     ...
     & /(za(k5,k6)*zb(k2,k1)*(sprod(k1,k3)+sprod(k1,k4))
     & *zab2(k2,k3,k4,k1)**2)

     &+0.5d0*(za(k3,k5)*zb(k4,k1)*(sprod(k1,k3)+sprod(k1,k4))
     & *zab2(k5,k3,k4,k2))
     & /(za(k5,k6)*zb(k2,k1)*zab2(k2,k3,k4,k1)**2)

     & +0.5d0*(t(k1,k3,k4)*za(k1,k5)**2*zb(k4,k1)**2)
     ...
     & *(sprod(k1,k2)+sprod(k2,k3)+sprod(k2,k4))*zab2(k1,k3,k4,k6))
     & /(zab2(k2,k3,k4,k1)**2)

     & -(za(k1,k3)*za(k1,k5)*zb(k4,k1)
     & *(sprod(k1,k2)+sprod(k2,k3)+sprod(k2,k4))
     & *zb(k6,k1)*sprod(k3,k4))/(zab2(k2,k3,k4,k1)**2)

     & -(za(k1,k2)*za(k3,k4)*zb(k4,k1)**2*zab2(k5,k3,k4,k1)
     & *zab2(k1,k3,k4,k2)
     & *(t(k1,k3,k4)*sprod(k1,k2)
     & +(sprod(k1,k3)+sprod(k1,k4))*(sprod(k1,k2)+sprod(k2,k3)+
     & sprod(k2,k4)))
     & *zb(k6,k1))
     & /(zb(k2,k1)*(sprod(k1,k3)+sprod(k1,k4))*zab2(k2,k3,k4,k1)**3)  
     ...
     & /((sprod(k1,k3)+sprod(k1,k4))*zab2(k2,k3,k4,k1)**2)  

     & +(2d0*za(k1,k3)*za(k1,k5)**2
     & *zb(k2,k1)*zb(k4,k1)*zb(k6,k5)*sprod(k3,k4))
     & /((sprod(k1,k3)+sprod(k1,k4))*zab2(k2,k3,k4,k1))

     & +(za(k1,k3)**2*za(k1,k5)**2*zb(k2,k1)*zb(k4,k3)*zb(k6,k5))
     & /(za(k1,k2)*(sprod(k1,k3)+sprod(k1,k4))))
     & /(sprod(k1,k2)*sprod(k3,k4)*sprod(k5,k6))
...
c      if (docheck) call ggZZcapture('1x34pp',h3,h5,j1,j2,j3,j4,j5,j6,
c     &                              app0,app2,czero)
